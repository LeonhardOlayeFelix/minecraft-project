...
const noRecipe = {
  item: "Empty",
  quantity: 1,
  recipe: ["Air", "Air", "Air", "Air", "Air", "Air", "Air", "Air", "Air"],
  shapeless: false,
};
const mergeItemData = (
...
const useBlocksAndItems = () => {
  const {
    data: anishItems,
    isLoading: anishItemsLoading,
    error: anishItemsError,
  } = useQuery({
  ...
  const {
    data: MinecraftDataItems,
    isLoading: MinecraftDataItemsLoading,
    error: MinecraftDataItemsError,
  } = useQuery({
  ...
  const {
    data: anishBlocks,
    isLoading: anishBlocksLoading,
    error: anishBlocksError,
  } = useQuery({
  ...
  const {
    data: MinecraftDataBlocks,
    isLoading: MinecraftDataBlocksLoading,
    error: MinecraftDataBlocksError,
  } = useQuery({
  ...
  const {
    data: anishRecipes,
    isLoading: anishRecipesLoading,
    error: anishRecipesError,
  } = useQuery({
  ...
  const {
    data: MinecraftDataFoods,
    isLoading: MinecraftDataFoodsLoading,
    error: MinecraftDataFoodsError,
  } = useQuery({
  ...
  const items =
    MinecraftDataItems && anishItems
      ? (mergeItemData(MinecraftDataItems, anishItems) as ItemsProps[])
      : [];
  const blocks =
    anishBlocks && MinecraftDataBlocks
      ? (mergeBlockData(anishBlocks, MinecraftDataBlocks) as BlocksProps[])
      : [];
  const recipes = anishRecipes
    ? [
        ...anishRecipes.map((recipe) =>
          recipe.item === "Beacon" ? beacon : recipe
        ),
        noRecipe,
      ]
    : [];
  const consumable =
    MinecraftDataFoods && anishItems
      ? (mergeFoodData(
          MinecraftDataFoods,
          anishItems as ItemsProps[]
        ) as FoodProps[])
      : [];

  const toolsAndWeaponry = items.filter((item) =>
    [
      "Chestplate",
      "Leggings",
      "Boots",
      ...
      "Arrow",
      "Potion",
    ].some((tool) => item.name.includes(tool))
  );
  const potions = items.filter((item) =>
    ["Potion", "Arrow of"].some((tool) => item.name.includes(tool))
  );
  ...
  const valuables = items.filter((item) =>
    [
  ...
  const musicDiscs = items.filter((item) =>
    ["Music Disc", "Jukebox", "Note Block"].some((tool) =>
  ...
  const isLoading =
    anishItemsLoading ||
    MinecraftDataItemsLoading ||
    anishBlocksLoading ||
    MinecraftDataBlocksLoading ||
    anishRecipesLoading ||
    MinecraftDataFoodsLoading;
  const error =
    anishItemsError ||
    MinecraftDataItemsError ||
    anishBlocksError ||
    MinecraftDataBlocksError ||
    anishRecipesError ||
    MinecraftDataFoodsError;
  return {
    items,
    blocks,
    potions,
