import { UseBlocksAndItemsResult } from "../../interfaces/MinecraftInterfaces";
...
const getItemsInCategory = (
  ...
  if (category === "Any") {
    ...
    filteredData = data.items.filter(
      (item) =>
        data.toolsAndWeaponry?.find((weapon) => weapon.name == item.name) !==
        undefined
    );
    ...
    filteredData = data.items.filter(
      (item) =>
        data.consumable?.find((consumable) => consumable.name == item.name) !==
        undefined
    );
    ...
    filteredData = data.items.filter(
      (item) =>
        data.valuables?.find((valuable) => valuable.name == item.name) !==
        undefined
    );
    ...
    filteredData = data.items.filter(
      (item) =>
        data.ingredients?.find((recipe) => recipe.name == item.name) !==
        undefined
    );
  }
  return filteredData;
